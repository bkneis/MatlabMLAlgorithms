...
for m=1:(ngener/n_show)
   for i=1:n_show
      ...
      % Mutation
      for i=1:numsel % Loop through number of chromosomes to be selected for reproduction (from crossover)
         if rand(1)<Pm % Check to see if to mutate the chromesone
            points=sort((round(rand(floor(numsel/2),1).*(ngenes-1))+1)');
            newchrom(i,:)=[newchrom(i,1:points(1)),... % Create new chromosome
               fliplr(newchrom(i,points(1)+1:points(2))),... % Flip the old bits with the new mutates bits
               newchrom(i,points(2)+1:ngenes)]; % Create the new mutates chromosone   
         end
      end
